...
class ReportVisualizer:
    ...
    def create_similarity_chart(
        ...
        colors = [
            "#e74c3c" if s >= threshold else "#f39c12" if s >= 0.3 else "#2ecc71"
            for s in scores
        ]

        bars = ax.bar(
            labels, scores, color=colors, alpha=0.8, edgecolor="black", linewidth=0.5
        )
    ...
    def create_distribution_chart(
        ...
        n, bins_edges, patches = ax.hist(
            scores, bins=bins, color="#3498db", edgecolor="black", alpha=0.7
        )
    ...
    def create_summary_dashboard(
        ...
        summary_text = [
            ...
            f"    • Average: {stats.get('average_similarity', 0):.2%}",
