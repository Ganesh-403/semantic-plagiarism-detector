...
def _cell(value: str) -> str:
    """Keep document names literal when a spreadsheet opens the CSV."""
    return (
        "'" + value
...
class CSVExporter:
    ...
    def export(self, report: Report, content: Dict[str, Any]) -> str:
        ...
        if report.statistics:
            ...
            stat_items = [
                ...
                ("Average Similarity", f"{stats.get('average_similarity', 0):.4f}"),
        ...
        if report.similarity_matrix and report.document_names:
            ...
            header = [""] + [_cell(name) for name in report.document_names]
            writer.writerow(header)

            # Data rows
            for i, row in enumerate(report.similarity_matrix):
                row_data = [_cell(report.document_names[i])] + [
        ...
        if report.matches:
            ...
            for i, match in enumerate(report.matches, 1):
                ...
                writer.writerow(
                    [
                        i,
                        _cell(match.get("source_document", "Unknown")),
                        _cell(match.get("target_document", "Unknown")),
                        f"{match['lexical_score']:.4f}"
                        if "lexical_score" in match
                        else "",
        ...
        for match in report.matches:
            ...
            writer.writerow(
                [
                    _cell(match.get("source_document", "Unknown")),
                    _cell(match.get("target_document", "Unknown")),
                    f"{match['lexical_score']:.4f}" if "lexical_score" in match else "",
                    f"{match['semantic_score']:.4f}"
                    if "semantic_score" in match
                    else "",
                    f"{match.get('hybrid_score', match.get('score', 0)):.4f}",
    ...
    def export_matrix_only(self, report: Report) -> str:
        ...
        # Data rows
        for i, row in enumerate(report.similarity_matrix):
            row_data = [_cell(report.document_names[i])] + [f"{val:.4f}" for val in row]
            writer.writerow(row_data)
