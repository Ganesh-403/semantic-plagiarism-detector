...
class PDFGenerator:
    ...
    def generate(self, report: Report, content: Dict[str, Any]) -> bytes:
        ...
        # Title page
        story.append(Paragraph(escape(report.title), self.styles["CustomTitle"]))
        ...
        if report.description:
            story.append(
                Paragraph(escape(report.description), self.styles["CustomBody"])
            )
        ...
        if report.similarity_matrix and report.document_names:
            ...
            img_data = content.get("visualizations", {}).get(
                "heatmap"
            ) or self.visualizer.create_heatmap(
            ...
            if img_data:
                try:
                    ...
                    story.append(
                        Paragraph(
                            "Heatmap generation failed.", self.styles["CustomBody"]
                        )
                    )
        ...
        if report.statistics:
            story.append(PageBreak())
            story.append(
                Paragraph("Detailed Statistics", self.styles["CustomHeading2"])
            )
            story.append(Spacer(1, 0.1 * inch))
        ...
        story.append(
            Paragraph(
                "Report generated by Semantic Plagiarism Detector",
                self.styles["CustomSmall"],
            )
        )
    ...
    def _create_matches_table(self, report: Report) -> Optional[Table]:
        ...
        for i, match in enumerate(matches, 1):
            ...
            data.append([str(i), source, target, f"{score:.2%}", severity])

        table = Table(
            data,
            colWidths=[0.4 * inch, 1.7 * inch, 1.7 * inch, 0.7 * inch, 0.7 * inch],
            repeatRows=1,
        )
        table.setStyle(
            TableStyle(
