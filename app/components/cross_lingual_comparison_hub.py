...
LANGUAGES = [
    ...
    ("Arabic", "ar"), ("Hindi", "hi"), ("Russian", "ru"), ("Italian", "it"),
]

DOC_PAIRS = [
    ("Research on Neural Machine Translation", "Investigación sobre Traducción Automática Neural", "en", "es"),
    ("Deep Learning for Text Analysis", "Analyse de Texte par Apprentissage Profond", "en", "fr"),
    ("Semantic Similarity Detection Methods", "Methoden zur Erkennung semantischer Ähnlichkeiten", "en", "de"),
    ("Plagiarism Detection in Multilingual Corpora", "多语言语料库中的抄袭检测", "en", "zh"),
    ("Automated Essay Scoring Systems", "自動採点システムの開発", "en", "ja"),
    ("Cross-lingual Transfer Learning Survey", "Cross-lingual Transfer Learningの調査", "en", "ja"),
    ("Natural Language Processing Advances", "Avances en Procesamiento de Lenguaje Natural", "en", "es"),
    ("Information Retrieval in Multiple Languages", "Recherche d'Information Multilingue", "en", "fr"),
    ("Text Classification Across Languages", "Textklassifizierung über Sprachen hinweg", "en", "de"),
    ("Machine Translation Quality Estimation", "质量评估机器翻译", "en", "zh"),
    ("Sentiment Analysis for Global Content", "Análisis de Sentimiento para Contenido Global", "en", "es"),
    ("Document Similarity in Cross-lingual Settings", "Similarité de Documents en Contexte Multilingue", "en", "fr"),
]
...
def generate_language_profiles() -> List[LanguageProfile]:
    ...
    for name, code in LANGUAGES[:8]:
        profiles.append(LanguageProfile(
            language=name, language_code=code,
            ...
            detected_scripts=["Latin", "Cyrillic", "CJK", "Arabic"] if code in ("ru", "zh", "ar", "ja", "ko") else ["Latin"],
        ))
    return profiles


def generate_translations() -> List[TranslationQuality]:
    translators = ["Google Translate", "DeepL", "Microsoft Translator", "Custom Model", "Human Translator"]
    qualities = []
    for src, tgt, sl, tl in DOC_PAIRS[:10]:
        t = random.choice(translators)
        bleu = random.uniform(25, 95)
        qual = "excellent" if bleu > 85 else "good" if bleu > 65 else "acceptable" if bleu > 45 else "poor"
        issues = []
        if bleu < 60:
        ...
        qualities.append(TranslationQuality(
            translator=t, source_lang=sl, target_lang=tl,
            ...
            issues=issues, quality_level=qual,
        ))
    return qualities


def generate_cross_lingual_matches() -> List[CrossLingualMatch]:
    match_types = ["direct_translation", "semantic_equivalent", "paraphrase_cross", "structural_clone"]
    matches = []
    for i, (src, tgt, sl, tl) in enumerate(DOC_PAIRS):
        sim = random.uniform(0.15, 0.95)
        risk = "critical" if sim > 0.85 else "high" if sim > 0.65 else "medium" if sim > 0.4 else "low"
        matches.append(CrossLingualMatch(
            match_id=f"CLM-{i+1:03d}",
            source_doc_id=f"doc_{random.randint(1,100)}", source_lang=sl,
            target_doc_id=f"doc_{random.randint(1,100)}", target_lang=tl,
            ...
            risk_level=risk,
            evidence={"exact_phrases": random.randint(1, 20), "structural_similarity": round(random.uniform(0.2, 0.9), 2)},
        ))
    return matches
...
def generate_cultural_contexts() -> List[CulturalContext]:
    contexts = [
        CulturalContext("CTX-001", "idiom", "It's raining cats and dogs", "en", "Llueven gatos y perros",
                       "Heavy rain is falling", "English", "Replace with regional equivalent"),
        CulturalContext("CTX-002", "proverb", "The early bird catches the worm", "en", "El que madruga, Dios le ayuda",
                       "Being proactive yields rewards", "English", "Use local proverb equivalent"),
        CulturalContext("CTX-003", "cultural_reference", "As American as apple pie", "en", "Tan español como la paella",
                       "Quintessentially American", "American", "Use culturally relevant local reference"),
        CulturalContext("CTX-004", "metaphor", "Break a leg", "en", "¡Mucha mierda!",
                       "Good luck before a performance", "English", "Use theater tradition of the target culture"),
        CulturalContext("CTX-005", "idiom", "C'est la vie", "fr", "That's life",
                       "Accepting life as it is", "French", "Direct translation works in this case"),
        CulturalContext("CTX-006", "humor", "Why do programmers prefer dark mode? Because light attracts bugs", "en",
                       "¿Por qué a los programadores les gusta el modo oscuro? Porque la luz atrae errores",
                       "Programming humor about bugs", "English", "Adapt joke structure for target language"),
        CulturalContext("CTX-007", "cultural_reference", "Hit a home run", "en", "Hacer un golazo",
                       "Achieve great success", "American Baseball → Soccer", "Replace sport metaphor"),
        CulturalContext("CTX-008", "proverb", "Marco ni marka", "es", "Beggars can't be choosers",
                       "Accept what you get", "Spanish", "Find equivalent English proverb"),
    ]
    return contexts
...
def generate_language_pair_stats() -> List[LanguagePairStats]:
    ...
    for sl, tl, docs, sim, pl, tq in pairs:
        stats.append(LanguagePairStats(
            source_lang=sl, target_lang=tl, docs_compared=docs,
            avg_similarity=round(sim, 3), plagiarism_rate=round(pl, 3),
            avg_translation_quality=round(tq, 3),
            common_patterns=["Literal translation", "False friends", "Register mismatch"],
            risk_factors=["Different script systems", "Cultural context loss", "Idiom handling"],
        ))
    return stats
...
def quality_color(q: str) -> str:
    return {"excellent": "#22c55e", "good": "#3b82f6", "acceptable": "#eab308", "poor": "#ef4444"}.get(q, "#6b7280")
...
def render_match_card(match: CrossLingualMatch) -> None:
    ...
    type_colors = {"direct_translation": "#ef4444", "semantic_equivalent": "#f97316", "paraphrase_cross": "#eab308", "structural_clone": "#8b5cf6"}
...
def render_cultural_context(ctx: CulturalContext) -> None:
    type_colors = {"idiom": "#8b5cf6", "proverb": "#22c55e", "cultural_reference": "#3b82f6", "metaphor": "#f59e0b", "humor": "#ec4899"}
...
def render_cross_lingual_hub() -> None:
    ...
    avg_bleu = sum(t.bleu_score for t in translations) / len(translations) if translations else 0
    cultures_detected = len(cultural)
    langs_covered = len(set(m.source_lang for m in matches) | set(m.target_lang for m in matches))

    cols = st.columns(6)
    kpis = [
        ("Comparisons", str(total_comparisons), "cross-lingual pairs", "#3b82f6"),
        ("High Risk", str(high_risk), "plagiarism risk", "#ef4444" if high_risk > 3 else "#22c55e"),
        ("Avg Similarity", f"{avg_sim:.0%}", "cross-lingual", sim_color(avg_sim)),
        ("Avg BLEU", f"{avg_bleu:.0f}", "translation quality", quality_color("good" if avg_bleu > 65 else "acceptable")),
    ...
    # Tabs
    tabs = ["📊 Overview", "🔄 Comparisons", "📝 Translations", "🎭 Cultural", "📈 Language Pairs"]
...
def _render_overview(profiles, matches, translations, pair_stats):
    ...
    with col2:
        ...
        for m in matches:
            if m.similarity_score < 0.2: sim_ranges["0-20%"] += 1
            elif m.similarity_score < 0.4: sim_ranges["20-40%"] += 1
            elif m.similarity_score < 0.6: sim_ranges["40-60%"] += 1
            elif m.similarity_score < 0.8: sim_ranges["60-80%"] += 1
            else: sim_ranges["80-100%"] += 1
        ...
        for ps in sorted(pair_stats, key=lambda x: x.docs_compared, reverse=True)[:5]:
            st.markdown(f"• **{ps.source_lang} → {ps.target_lang}**: {ps.docs_compared} docs, {ps.avg_similarity:.0%} avg sim")
...
def _render_comparisons(matches):
    ...
    with col2:
        type_filter = st.selectbox("Match Type", ["All"] + list(set(m.match_type for m in matches)))
    with col3:
        sort_by = st.selectbox("Sort By", ["Similarity (High→Low)", "Similarity (Low→High)", "Segments"])
...
def _render_language_pairs(pair_stats, profiles):
    ...
    for ps in pair_stats:
        ...
        pqc = quality_color("good" if ps.avg_translation_quality > 0.7 else "acceptable")
        st.markdown(f"""
        <div style="background:rgba(255,255,255,0.06);border-radius:12px;padding:14px;
             border:1px solid rgba(255,255,255,0.08);margin-bottom:10px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
