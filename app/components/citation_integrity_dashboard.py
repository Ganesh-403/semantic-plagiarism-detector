...
class CitationAnomaly:
    ...
    anomaly_id: str
    anomaly_type: str  # self_citation_burst, citation_ring, fake_source, temporal_cluster
...
def generate_citations(count: int = 30) -> List[Citation]:
    ...
    journals = [
        "Nature", "Science", "IEEE Transactions", "ACM Computing Surveys",
        "Journal of Machine Learning Research", "Physical Review Letters",
        "The Lancet", "Cell", "PNAS", "Nature Communications",
        "arXiv preprint", "NeurIPS Proceedings", "ICML Proceedings",
    ]
    ...
    doc_titles = [
        "Deep Learning for NLP", "Transformer Architecture Analysis",
        "Semantic Similarity Methods", "Plagiarism Detection Survey",
        "Code Clone Detection", "AI-Generated Text Identification",
        "Cross-lingual Transfer Learning", "Knowledge Graph Completion",
        "Information Retrieval Methods", "Text Summarization Techniques",
    ]
    ...
    for i in range(count):
        ...
        citations.append(Citation(
            ...
            source_doi=f"10.{random.randint(1000, 9999)}/{random.randint(10000, 99999)}" if random.random() > 0.2 else None,
...
def generate_authenticity(citations: List[Citation]) -> List[SourceAuthenticity]:
    ...
    for c in citations:
        ...
        results.append(SourceAuthenticity(
...
def generate_anomalies() -> List[CitationAnomaly]:
    ...
    anomalies = [
        CitationAnomaly(
            anomaly_id="ANOM-001", anomaly_type="self_citation_burst",
            severity="high",
            description="Document 'Deep Learning for NLP' has 45% self-citation rate, exceeding the 20% threshold.",
            affected_docs=["doc_1"], evidence={"self_cite_rate": 0.45, "threshold": 0.20},
            detected_at=(datetime.now() - timedelta(days=2)).isoformat(), status="investigating",
        ),
        CitationAnomaly(
            anomaly_id="ANOM-002", anomaly_type="citation_ring",
            severity="critical",
            ...
            detected_at=(datetime.now() - timedelta(days=1)).isoformat(), status="open",
        ),
        CitationAnomaly(
            anomaly_id="ANOM-003", anomaly_type="fake_source",
            severity="critical",
            description="DOI 10.99999/99999 resolves to a non-existent paper. Likely fabricated citation.",
            affected_docs=["doc_2"], evidence={"doi": "10.99999/99999", "http_status": 404},
            detected_at=(datetime.now() - timedelta(hours=6)).isoformat(), status="open",
        ),
        CitationAnomaly(
            anomaly_id="ANOM-004", anomaly_type="temporal_cluster",
            severity="medium",
            ...
            detected_at=(datetime.now() - timedelta(hours=12)).isoformat(), status="resolved",
        ),
        CitationAnomaly(
            anomaly_id="ANOM-005", anomaly_type="self_citation_burst",
            severity="medium",
            ...
            detected_at=(datetime.now() - timedelta(days=5)).isoformat(), status="dismissed",
        ),
    ]
    return anomalies
...
def generate_network() -> List[CitationNetwork]:
    ...
    return [
        CitationNetwork(
            doc_id=d[0], doc_title=d[1], citation_count=d[2],
            cited_by_count=d[3], self_citation_rate=d[4],
            avg_source_quality=d[5], top_journals=journals_map[d[0]],
...
def severity_color(severity: str) -> str:
    return {"critical": "#ef4444", "high": "#f97316", "medium": "#eab308", "low": "#22c55e"}.get(severity, "#6b7280")


def status_color(status: str) -> str:
    return {"open": "#ef4444", "investigating": "#f59e0b", "resolved": "#22c55e", "dismissed": "#94a3b8"}.get(status, "#6b7280")
...
def render_citation_card(citation: Citation) -> None:
    ...
    flag_badge = '<span style="background:#ef444420;color:#ef4444;padding:2px 8px;border-radius:12px;font-size:10px;font-weight:600;">⚠️ Flagged</span>' if citation.flagged else ''
    verified_badge = '<span style="background:#22c55e20;color:#22c55e;padding:2px 8px;border-radius:12px;font-size:10px;font-weight:600;">✅ Verified</span>' if citation.verified else ''
...
def render_network_node(net: CitationNetwork) -> None:
    ...
    self_rate_color = "#ef4444" if net.self_citation_rate > 0.3 else "#eab308" if net.self_citation_rate > 0.15 else "#22c55e"
    st.markdown(f"""
...
def render_citation_integrity_dashboard() -> None:
    ...
    avg_authenticity = sum(a.authenticity_score for a in authenticity) / len(authenticity)
    retracted_count = sum(1 for a in authenticity if a.retracted)

    kpi_cols = st.columns(6)
    kpis = [
        ("Citations", str(len(citations)), "total tracked", "#3b82f6"),
        ("Verified", f"{verified_count}/{len(citations)}", f"{verified_count/len(citations):.0%} verified", "#22c55e"),
        ("Flagged", str(flagged_count), "need review", "#ef4444" if flagged_count > 3 else "#eab308"),
        ("Avg Confidence", f"{avg_confidence:.0%}", "citation confidence", confidence_color(avg_confidence)),
        ("Anomalies", str(open_anomalies), "open issues", "#ef4444" if open_anomalies > 1 else "#22c55e"),
        ("Authenticity", f"{avg_authenticity:.0%}", f"{retracted_count} retracted", confidence_color(avg_authenticity)),
    ]
    ...
    selected_tab = st.radio("Dashboard Tabs", tabs, horizontal=True, label_visibility="collapsed")
...
def _render_citations_tab(citations: List[Citation]) -> None:
    ...
    with col1:
        type_filter = st.selectbox("Citation Type", ["All"] + list(set(c.citation_type for c in citations)))
    with col2:
        status_filter = st.selectbox("Status", ["All", "Verified", "Flagged", "Unverified"])
    with col3:
        sort_by = st.selectbox("Sort By", ["Confidence", "Similarity", "Relevance", "Year"])
...
def _render_authenticity_tab(authenticity: List[SourceAuthenticity]) -> None:
    ...
    for auth in authenticity:
        ...
        checks = [
            ...
            ("✅ Accessible" if auth.url_accessible else "❌ Inaccessible", auth.url_accessible),
            ("✅ Metadata Match" if auth.metadata_match else "❌ Mismatch", auth.metadata_match),
            ("✅ Peer Reviewed" if auth.peer_reviewed else "⚠️ Not Peer Reviewed", auth.peer_reviewed),
            ("✅ Not Retracted" if not auth.retracted else "🚫 RETRACTED", not auth.retracted),
            ("✅ Legitimate Journal" if not auth.predatory_journal else "⚠️ Predatory", not auth.predatory_journal),
        ]
        checks_html = "".join(f'<span style="font-size:10px;color:{"#22c55e" if ok else "#ef4444"};">{label}</span>' for label, ok in checks)
...
def _render_anomalies_tab(anomalies: List[CitationAnomaly]) -> None:
    ...
    with col2:
        ...
        for stat, count in stat_counts.most_common():
            ...
            st.markdown(f'<div style="font-size:12px;color:{col};margin-bottom:4px;">{stat.title()}: <b>{count}</b></div>', unsafe_allow_html=True)


def _render_network_tab(network: List[CitationNetwork], citations: List[Citation]) -> None:
