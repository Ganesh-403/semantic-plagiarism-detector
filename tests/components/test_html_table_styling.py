...
from src.utils.processing_time import ProcessingTimer, TimingUIRenderer
from app.components.report_generator import PlagiarismReportGenerator, ReportConfig, ReportData
import pandas as pd
...
def test_processing_time_css_injection():
    ...
    """
    css = TimingUIRenderer._generate_css(is_dark_mode=False)
    ...
    css_dark = TimingUIRenderer._generate_css(is_dark_mode=True)
...
def test_processing_time_render_expander_table_structure():
    ...
    TimingUIRenderer.render_debug_expander(timer, is_dark_mode=False, st_module=st_mock)
...
def test_report_generator_table_styles():
    ...
    """
    generator = PlagiarismReportGenerator(ReportConfig(output_format="html"))
    data = ReportData("2026-01-01", 2, 1, 0, .2, .2, .2, [], pd.DataFrame(),
                      ["a.txt", "b.txt"], .75, .1, {"version": "test"}, [])
    report_html = generator.generate_report(data)
...
def test_automatic_report_generation_html_structure():
    ...
    for template_id in generator.templates:
        html = generator.generate_report(template_id, sample_data).content
        soup = BeautifulSoup(html, "html.parser")
        ...
        tables = soup.find_all("table")
        # Summary-only templates legitimately render metrics without a table.
        if template_id == "detailed_report":
            assert len(tables) > 0
        for table in tables:
