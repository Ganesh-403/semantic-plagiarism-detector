...
def test_render_document_comparison_highlight_overlap():
    ...
    with patch("streamlit.columns") as mock_cols, \
        ...
        mock_col = MagicMock()
        mock_cols.return_value = [mock_col, mock_col]
        mock_text_area.side_effect = ["This is a document content.", "This is another document content."]

        render_document_comparison()
        
        # Verify markdown highlights are printed
        calls = [args[0] for args, _ in mock_markdown.call_args_list if isinstance(args[0], str)]
