...
from src.api.routers.analysis import scan_jobs, _process_scan_job
...
def test_exact_hash_match_fast_path(tmp_path):
    ...
    with mock.patch("src.api.routers.analysis.calculate_file_sha256") as mock_calc:
        with mock.patch("src.api.routers.analysis.get_document_by_hash") as mock_get_hash:
            with mock.patch("src.api.routers.analysis.embed_chunks") as mock_embed:
                with mock.patch("src.api.routers.analysis.get_document_embedding") as mock_doc_embed:
...
def test_no_hash_match_fallback(tmp_path):
    ...
    with mock.patch("src.api.routers.analysis.calculate_file_sha256") as mock_calc:
        with mock.patch("src.api.routers.analysis.get_document_by_hash") as mock_get_hash:
            with mock.patch("src.api.routers.analysis.embed_chunks") as mock_embed:
                with mock.patch("src.api.routers.analysis.get_document_embedding") as mock_doc_embed:
                    with mock.patch("src.api.routers.analysis.get_corpus_documents_with_embeddings") as mock_corpus:
                        mock_calc.return_value = "fake_sha256"
