...
def test_backup_daemon_waits_for_timeout_after_startup():
    ...
    with patch("app.state_manager.get_cache", return_value=mock_cache), \
         ...
         patch("src.db.database_backup.create_corpus_database_snapshot") as mock_snapshot, \
...
def test_backup_daemon_triggers_after_timeout_after_startup():
    ...
    with patch("app.state_manager.get_cache", return_value=mock_cache), \
         patch("src.core.app_config.get_backup_idle_timeout", return_value=1800), \
         patch("app.state_manager.get_active_sessions_count", return_value=0), \
         patch("src.db.database_backup.create_corpus_database_snapshot", return_value=b"snap") as mock_snapshot, \
         patch("src.db.database_backup.cleanup_old_backups"), \
         patch("src.core.app_config.get_backup_dir"), \
         patch("time.sleep", side_effect=InterruptedError("Stop loop")):
         
         current_time = time.time()
         # Force time to advance by 1801 seconds, which exceeds the timeout!
         with patch("time.time", side_effect=[current_time, current_time + 1801, current_time + 1801]):
             try:
                 _run_backup_daemon()
             except InterruptedError:
                 pass
...
def test_backup_daemon_missing_last_activity_on_startup():
    ...
    with patch("app.state_manager.get_cache", return_value=mock_cache), \
         ...
         with patch("time.time", side_effect=[current_time, current_time + 2000, current_time + 2000]):
             try:
                 _run_backup_daemon()
             except InterruptedError:
                 pass
             ...
             mock_cache.set.assert_called_with("spd:v1:global:last_activity", current_time + 2000)
             mock_snapshot.assert_not_called()
...
def test_backup_daemon_startup_with_active_sessions():
    ...
    with patch("app.state_manager.get_cache", return_value=mock_cache), \
         patch("src.core.app_config.get_backup_idle_timeout", return_value=1800), \
         patch("app.state_manager.get_active_sessions_count", return_value=3), \
         patch("src.db.database_backup.create_corpus_database_snapshot") as mock_snapshot, \
         patch("time.sleep", side_effect=InterruptedError("Stop loop")):
...
def test_backup_daemon_exactly_at_timeout_boundary():
    ...
    with patch("app.state_manager.get_cache", return_value=mock_cache), \
         ...
         current_time = time.time()
         with patch("time.time", side_effect=[current_time, current_time + 1800, current_time + 1800]):
             try:
                 _run_backup_daemon()
             except InterruptedError:
                 pass
...
def test_backup_daemon_transition_from_active_to_zero_sessions():
    ...
    with patch("app.state_manager.get_cache", return_value=mock_cache), \
         patch("src.core.app_config.get_backup_idle_timeout", return_value=1800), \
         patch("app.state_manager.get_active_sessions_count", side_effect=[3, 0]), \
         patch("src.db.database_backup.create_corpus_database_snapshot", return_value=b"snap") as mock_snapshot, \
         patch("src.db.database_backup.cleanup_old_backups"), \
         patch("src.core.app_config.get_backup_dir"), \
         patch("time.sleep", side_effect=[None, InterruptedError("Stop loop")]):
         ...
         with patch("time.time", side_effect=[
             current_time,             # daemon_start_time
             current_time + 2000,      # First iteration now() (fails on active_sessions == 3)
...
def test_backup_daemon_startup_phase_skips_backup_but_updates_activity():
    ...
    with patch("app.state_manager.get_cache", return_value=mock_cache), \
         ...
         with patch("time.time", side_effect=[current_time, current_time + 100]):
             ...
             mock_cache.set.assert_called_with("spd:v1:global:last_activity", current_time + 100)
             mock_snapshot.assert_not_called()
...
def test_backup_daemon_recovers_from_cache_error_gracefully():
    ...
    with patch("app.state_manager.get_cache", return_value=mock_cache), \
         patch("src.core.app_config.get_backup_idle_timeout", return_value=1800), \
         patch("time.sleep", side_effect=[None, InterruptedError("Stop loop")]) as mock_sleep:
...
def test_backup_daemon_mocked_time_progression():
    ...
    with patch("app.state_manager.get_cache", return_value=mock_cache), \
         ...
         patch("src.db.database_backup.cleanup_old_backups"), \
         patch("src.core.app_config.get_backup_dir"), \
         patch("time.sleep", side_effect=[None, None, None, InterruptedError("Stop loop")]):
