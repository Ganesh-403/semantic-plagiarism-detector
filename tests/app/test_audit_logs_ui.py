...
def temp_audit_db(tmp_path):
    ...
    # Seed test events
    log_security_event("login", "admin", "Admin login successful")
    log_security_event("file_upload", "teacher1", "Uploaded document assignment.pdf")
    log_security_event("password_change", "teacher1", "Password updated successfully")
    log_security_event("token_revocation", "system", "Revoked bearer token")
...
def test_audit_logs_query_and_count(temp_audit_db):
    ...
    count = get_security_audit_log_count()
    assert count >= 4

    logs = get_security_audit_logs(limit=10)
    assert len(logs) >= 4

    event_types = get_distinct_audit_event_types()
...
def test_audit_logs_username_filter(temp_audit_db):
    ...
    teacher_logs = get_security_audit_logs(username="teacher1")
...
def test_audit_logs_event_type_filter(temp_audit_db):
    ...
    login_logs = get_security_audit_logs(event_type="login")
