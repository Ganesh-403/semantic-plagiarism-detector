...
from src.core.document_parser import extract_text_from_pdf, extract_text_from_docx
...
from src.core.similarity import compute_similarity_matrix, flag_plagiarism, classify_severity
...
class TestCompletePlagiarismScanLifecycle:
    ...
    def test_full_scan_workflow_single_document(
        ...
        for chunk in all_chunks:
            faiss_index.add(chunk['embedding'], metadata={'doc_id': chunk['document_id']})
    ...
    def test_plagiarism_detection_accuracy(
        ...
        assert sim_plagiarized >= PLAGIARISM_THRESHOLD, "Plagiarized content should be flagged"
...
class TestE2EWithRealPDFs:
    ...
    def test_pdf_upload_to_results_workflow(self, corpus_db, test_data_dir):
        ...
        with patch('src.core.document_parser.extract_text_from_pdf') as mock_extract:
            mock_extract.return_value = "This is a test document for plagiarism detection"
...
class TestE2EWithConfiguration:
    ...
    def test_threshold_configuration_affects_flags(
        ...
        assert flag_counts[-1][1] is False, "High threshold should not flag lower similarity"
    ...
    def test_configurable_chunk_size_affects_results(
        ...
        from src.core.config import CHUNK_MIN_WORDS, CHUNK_MAX_WORDS
