...
class TestIndexPerformance:
    def test_query_uses_index(self):
        ...
        # Insert sample data
        conn.execute("INSERT INTO documents (file_hash, title) VALUES (?, ?)", ("hash1", "Doc1"))
        conn.execute("INSERT INTO documents (file_hash, title) VALUES (?, ?)", ("hash2", "Doc2"))
        conn.commit()
        
        # Force index usage by analyzing the query plan
        cursor = conn.execute("EXPLAIN QUERY PLAN SELECT * FROM documents WHERE file_hash = ?", ("hash1",))
        plan = cursor.fetchall()
        
        # Check if any part of the query plan mentions the index
        plan_text = str(plan)
        assert "idx_documents_file_hash" in plan_text, f"Query did not use the index: {plan_text}"
...
class TestDataIntegrity:
    ...
    def test_can_query_by_hash(self):
        ...
        cursor = conn.execute("SELECT title FROM documents WHERE file_hash = ?", ("hash1",))
    ...
    def test_hash_is_unique_requirement(self):
        ...
        with pytest.raises(sqlite3.IntegrityError):
            ...
            conn.execute("INSERT INTO documents (file_hash, title) VALUES (?, ?)", ("hash1", "Doc2"))
        conn.close()
...
class TestMultipleFiles:
    ...
    def test_index_works_with_data(self, tmp_path):
        ...
        conn2 = sqlite3.connect(db_path)
        cursor = conn2.execute("SELECT title FROM documents WHERE file_hash = ?", ("hash1",))
        result = cursor.fetchone()
